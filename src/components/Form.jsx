...
const MINUTES_5 = ['00', '30', '05', '10', '15', '20', '25', '35', '40', '45', '50', '55'];
...
const TOTAL_FORMULA_FIELDS = ['amount', 'overtime', 'extendHr', 'percentageDiscount', 'otherDiscount'];
...
function parseTime(value) {
  ...
  const snapped = Number.isNaN(minNum) ? '' : (Math.round(minNum / 5) * 5 % 60).toString().padStart(2, '0');
...
function computeAmount(date, noOfPpl, noOfHrs, roomType = 'small') {
  ...
  const includedHours = roomType === 'large' ? AMOUNT_PRICE_LARGE.INCLUDED_HOURS : AMOUNT_PRICE_SMALL.INCLUDED_HOURS;
  if (roomType === 'large') {
    basePrice = isMonThu ? AMOUNT_PRICE_LARGE.BASE_MON_THU : AMOUNT_PRICE_LARGE.BASE_FRI_SUN;
    extraPerHourPerPerson = isMonThu ? AMOUNT_PRICE_LARGE.EXTRA_HOUR_PER_PERSON_MON_THU : AMOUNT_PRICE_LARGE.EXTRA_HOUR_PER_PERSON_FRI_SUN;
  } else {
    if (ppl >= 3) {
      basePrice = isMonThu ? AMOUNT_PRICE_SMALL.BASE_3_PPL_UP_MON_THU : AMOUNT_PRICE_SMALL.BASE_3_PPL_UP_FRI_SUN;
      extraPerHourPerPerson = isMonThu ? AMOUNT_PRICE_SMALL.EXTRA_HOUR_PER_PERSON_MON_THU : AMOUNT_PRICE_SMALL.EXTRA_HOUR_PER_PERSON_FRI_SUN;
    } else {
...
function computeTotal(amount, overtime, extendHr, percentageDiscount, otherDiscount) {
...
function getInitialState() {
  ...
  state.total = computeTotal(state.amount, state.overtime, state.extendHr, state.percentageDiscount, state.otherDiscount);
  return state;
}
...
export default function Form() {
  ...
  const [percentageDiscountIsOther, setPercentageDiscountIsOther] = useState(false);
  ...
  function handleChange(id, value) {
    setFormData((prev) => {
      ...
      if (['date', 'noOfPpl', 'startTime', 'endTime'].includes(id)) {
        const amount = computeAmount(next.date, next.noOfPpl, next.noOfHrs, roomType);
  ...
  function handleSubmit(e) {
    ...
    FORM_FIELDS.forEach(({ id, defaultZero, type }) => {
      if (defaultZero && (payload[id] === '' || payload[id] === undefined || payload[id] === null)) payload[id] = 0;
    ...
    const numericField = FORM_FIELDS.find((f) => f.type === 'number' && Number(payload[f.id]) < 0);
    if (numericField) {
      setStatus({ type: 'error', message: `${numericField.label} cannot be negative` });
      return;
    }
  ...
  function handleRoomTypeChange(newRoomType) {
    ...
    setFormData((prev) => {
      const next = { ...prev };
      const amount = computeAmount(prev.date, prev.noOfPpl, prev.noOfHrs, newRoomType);
      if (amount !== null) next.amount = amount;
      next.total = computeTotal(
        next.amount,
        next.overtime,
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        <div className={styles.roomToggle} role="tablist" aria-label="房間">
          <button
            ...
            className={roomType === 'small' ? styles.roomTabActive : styles.roomTab}
          ...
          <button
            ...
            className={roomType === 'large' ? styles.roomTabActive : styles.roomTab}
            onClick={() => handleRoomTypeChange('large')}
          >
            大房
          </button>
        </div>
        {FORM_FIELDS.filter(({ id }) => id !== 'depositReturned').map(({ id, label, type, required }) => (
              ...
              <div className={styles.timeRow}>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleChange(id, hour && minute ? `${hour}:${minute}` : hour);
                  }}
                  required={required}
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleChange(id, hour && minute ? `${hour}:${minute}` : minute);
                  }}
                  required={required}
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (v === 'other') {
                      ...
                      handleChange(id, PRESET_PERCENTAGES.includes(Number(current)) ? 0 : current);
                    } else {
                      setPercentageDiscountIsOther(false);
                  ...
                  aria-label={label}
                >
                  {PERCENTAGE_DISCOUNT_OPTIONS.map(({ value, label: optLabel }) => (
